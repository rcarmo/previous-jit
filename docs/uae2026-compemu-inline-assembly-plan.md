...
The opcode harness remains a reliable green gate for the current curated vector set (`pass=75 fail=0 score=100`; latest `/workspace/tmp/previous-opcode-harness-20260526-132114`), the RAM-code MMU fast smoke remains clean from `0x04008000` (`pass=32 fail=0 score=100`; latest `/workspace/tmp/previous-mmu-fast-smoke-20260526-132114`), and default/ROM JIT boot reaches the NEXTSTEP desktop in the latest smoke (`/workspace/tmp/previous-jit-bsr-metadata-default-20260526-132634`). RAM-requested mode now preserves native translated execution unless the explicit conservative oracle `B2_JIT_RTE_FAULT_HANDOFF=1` is requested; that oracle boots to a stable desktop in the latest long/no-DC run (`/workspace/tmp/previous-jit-bsr-metadata-ram-handoff-long-20260526-133132`). The current blocker is therefore narrower than generic opcode bring-up: native JIT resume after the RTE/page-fault seam. Commit `9441c84` aligned fallback BSR call-push transaction metadata, but native no-handoff still times out after RTE/low-PC churn.
