...
- The earlier low-ROM probe frontier (`00003352`), the shifted-stack `00003964/A2=00000002` loop, the `init exited with 212` path, the panic-monitor `bad exception stack format` failure, the repeated `000000de` loop, and the high-user `050abffe`/`050069cc` frontiers are no longer the active blockers. Fallback BSR call-push transactions pass producer-side decoded targets into the bridge instead of bridge-side extension decoding through transient `regs.pc_p`. Native no-handoff still has no desktop-reaching proof; current changes should be judged first by the bounded opcode/MMU/fault gates rather than by a fresh long-run frontier.
