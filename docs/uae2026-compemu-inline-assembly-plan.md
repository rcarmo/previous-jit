...
## 2026-05-19 Previous RAM/MMU checkpoint

The opcode harness remains a reliable green gate for the current curated vector set (`pass=62 fail=0 score=100`; latest `/workspace/tmp/previous-opcode-harness-20260520-180203`), and default/ROM JIT boot reaches the NEXTSTEP desktop with a 60s stability wait (`/workspace/tmp/previous-jit-doc-update-default-20260520-185848`). RAM-requested mode now reaches true RAM dispatch and boots to a stable desktop by default through a conservative RTE/page-fault handoff to the exact interpreter (`/workspace/tmp/previous-jit-doc-update-ram-handoff-20260520-190438`). The current blocker is therefore narrower than generic opcode bring-up: native JIT resume after the RTE/page-fault seam, reproducible with `B2_JIT_RTE_FAULT_HANDOFF_DISABLE=1`.
...
- `RTE` remains a hard seam: the native helper routes through the exact interpreter, and default RAM mode hands bridge-caught RTE/page-fault seams to the interpreter. `B2_JIT_RTE_FAULT_HANDOFF_DISABLE=1` must remain available until native resume is fixed.
...
- The earlier low-ROM probe frontier (`00003352`), the shifted-stack `00003964/A2=00000002` loop, the `init exited with 212` path, and the panic-monitor `bad exception stack format` failure are no longer the active blockers. With the RTE handoff disabled, narrow JSR call-push transaction producers for `0000003e`, `00003c26`, and `0000c52c` move the path past the repeated `000000de` loop; the latest completed discriminator stalls later around `050abffe` / `050ac000` (`/workspace/tmp/previous-jit-3c26-jsr-txn-disable-ram-20260520-172348`).
...
Immediate harness gaps: add minimal RAM/MMU regressions for return-target MMU faults after `RTS`/`RTR`, for RTE return-code fetch after SR/A7 switch, and for the handoff-disabled `050abffe` RAM-boundary/code-fetch loop.
