...
The opcode harness remains a reliable green gate for the current curated vector set (`pass=74 fail=0 score=100`; latest `/workspace/tmp/previous-opcode-harness-20260524-081241`), the RAM-code MMU fast smoke remains clean from `0x04008000` (`pass=31 fail=0 score=100`; latest `/workspace/tmp/previous-mmu-fast-smoke-20260524-081030`), and default/ROM JIT boot reaches the NEXTSTEP desktop in the latest no-DC smoke (`/workspace/tmp/previous-jit-lowpcdiag-default-20260523-202501`). RAM-requested mode now preserves native translated execution unless the explicit conservative oracle `B2_JIT_RTE_FAULT_HANDOFF=1` is requested; that oracle reaches true RAM dispatch and boots to a stable desktop (`/workspace/tmp/previous-jit-explicit-handoff-ram-20260522-090029`). The current blocker is therefore narrower than generic opcode bring-up: native JIT resume after the RTE/page-fault seam, compared against the handoff oracle with default-off low-PC code-host discriminators.
...
- The earlier low-ROM probe frontier (`00003352`), the shifted-stack `00003964/A2=00000002` loop, the `init exited with 212` path, the panic-monitor `bad exception stack format` failure, the repeated `000000de` loop, and the high-user `050abffe`/`050069cc` frontiers are no longer the active blockers. With `B2_JIT_LOW83_CODEHOST=1 B2_JIT_LOW7F_CODEHOST=1`, native no-handoff now matches the explicit-handoff oracle through the first nine low-PC catches (`00003344`, `00012b04`, `0000c572`, `0000e000`, `0000a8f6`, `00008306`, `00007f72`, `00008b14`, `00008b24`) before diverging with an extra native-only `0000ee58` catch (`addr=0001402a`).
...
Immediate harness gaps: add minimal RAM/MMU regressions for return-target MMU faults after `RTS`/`RTR`, for RTE return-code fetch after SR/A7 switch, and for the post-RTE low-PC resume sequence that currently matches the oracle through `00008b24` before native reaches the extra `0000ee58` catch.
