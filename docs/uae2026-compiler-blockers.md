...
3. add targeted regressions for the confirmed RAM-mode blockers: return-target MMU faults after `RTS`/`RTR`, RTE return-code fetch after SR/A7 switch, and the post-RTE low-PC native-resume sequence that currently matches the oracle through `00008b24` before native diverges at `0000ee58`
...
5. audit bridge/JIT state materialization before `Exception(2)` and before resuming JIT after an RTE/page-fault seam, especially PC/SR/USP/ISP, published restart flags, fetched opcode state, live D/A register spill, and the continuation/return state after the `00008b24` data fault
