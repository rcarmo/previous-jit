...
- low-virtual code-fetch discriminator: `B2_JIT_LOW_VIRTUAL_SINGLESTEP=1` now single-steps only the confirmed ROM probe window (`0x00003200..0x00003400`) by default through a real 040 MMU opcode fetch plus `cpufunctbl`, with optional `B2_JIT_LOW_VIRTUAL_SINGLESTEP_START/END` for range bisection. This eliminates the repeated `00003352`/`addr=00000008` loop and reaches `root on sd@` in RAM mode, but still no desktop within 600/1200s; widening the range toward `0x00014000` exposes a later post-root stall at `0401da6c`, mapped to an FPU/math compare branch in kernel text. The helper publishes `instruction_pc`/`fault_pc` before faultable opcode fetch, clears `mmu_opcode` to `0xffff` before the fetch to match the interpreter's instruction-fetch-fault state, and uses the normal `cpu_check_ticks()` path after interpreter execution.
- native low-virtual prefetch guard discriminator: `B2_JIT_LOW_VIRTUAL_PREFETCH_GUARD=1` performs only the interpreter-style 040 opcode fetch/restart-state publication before native low-virtual JIT ops in the same default `0x00003200..0x00003400` window (optional `B2_JIT_LOW_VIRTUAL_PREFETCH_START/END`). Audit follow-up publishes the opcode actually returned by `Uae2026JitMmuFetchOpcode()` after a successful fetch (rather than the compile-time literal) so any later native data fault has interpreter-like restart opcode state; `B2_JIT_TRACE_PREFETCH_GUARD=1` logs unexpected fetched/compiled opcode mismatches. This does not eliminate all `00003352` exceptions, but it lets RAM mode progress past them to `root on sd@` without interpreter single-stepping the low-ROM instructions; no desktop yet. Validation stayed green with the guard code default-off (`/workspace/tmp/previous-opcode-harness-prefetch-guard-audit-20260514-150218`, `/workspace/tmp/previous-jit-prefetch-guard-audit-default-stable-20260514-151230`) and the guard-on RAM discriminator still reaches `root on sd@` with no opcode mismatches (`/workspace/tmp/previous-jit-prefetch-guard-audit-ram-20260514-151804`, `jit_ram_dispatch_seen=1`, `jit_dispatch_lines=169836`, `jit_last_pc=0405840c`).
