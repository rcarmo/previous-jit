...
- explicit handoff RAM-requested mode historically reaches and holds the desktop when `B2_JIT_RTE_FAULT_HANDOFF=1` is set (`/workspace/tmp/previous-jit-bsr-metadata-ram-handoff-long-20260526-133132`: `desktop_reached=1`, `stable_reached=1`, `jit_ram_requested=1`, `harness_rc=0`); it was not rerun under the current 120s cap.
- automatic RAM-mode RTE handoff remains removed, so `PREVIOUS_UAE2026_JIT_RAM=1` preserves the native 100%-JIT path unless `B2_JIT_RTE_FAULT_HANDOFF=1` is explicitly set. Native no-handoff is not fixed. The old `000042f8` / `addr=0000020c` loop was traced to `0000aef4` MOVEM continuation-frame effective-address corruption and is now gone; current status is tracked by the bounded default/MMU/fault gates above rather than by a fresh long desktop run.
...
- bridge-side transaction coverage now includes explicit `call_push` producers for BSR paths and return-pop producers for `RTS`/`RTR` fallback paths; fallback BSR producers pass decoded targets into the bridge as of `9441c84`, avoiding extension-word re-decode through transient `regs.pc_p`; the proven historical `00003372/00003374 -> 00012b04` BSR seam still keeps its legacy compatibility scan until a producer covers that exact shifted-PC/native path
...
- current code checkpoint keeps the bridge-side call-push transaction producer for selected JSR target forms, but fallback BSR paths now pass producer-side decoded byte/word/long targets to `Uae2026JitMmuTxnBeginCallPushPreTarget(...)`; this removes the stale `regs.pc_p` metadata hazard seen at `00016906 -> 00012052` without changing target-fetch side-effect policy. The legacy BSR compatibility scan remains forward-only, and RAM code-shadow synchronization in `Uae2026JitMmuXlateCodeHost()` remains widened to 8 KiB so page-boundary translated code fetches see coherent shadow bytes.
- recent native-resume fixes kept target-fetch canonicalization narrow to the confirmed user `05027706` `JSR (A0)` seam, added bridge PC advancement for confirmed non-restartable byte-store faults at `0500b6ae`/`0500bc98`, and preserve the 68040 `MMU_SSW_CM` MOVEM continuation effective address instead of replacing it with `mmu_fault_addr`. The corresponding normal side-effect shapes are covered by RAM-code fast vectors; the old low42 `A3=0` symptom is cleared. Current no-handoff work is gated by bounded opcode/MMU/fault discriminators rather than by a fresh long frontier; the sampled `0409f592/0409f5cc` high-kernel polling loop did not move under `B2_JIT_EXACT_EXEC_PCS=0x0409f570-0x0409f5d0` and is treated as a symptom.
- post-RTE low-virtual and high-user opcode fetch now use code-host bytes for confirmed non-identity RAM/MMU mappings while keeping early ROM/low-overlay SCSI boot on the legacy 040 path. Covered default windows: isolated post-kernel low-PC fetch (`vbr=040ae61c`, `00003300..00003400`) and high user mappings (`05000000..07ffffff`) where `pc_p` translates to a different physical page. This avoids stale data-view streams such as `00003334` (`0200/0c80` vs `PCTSHADOW=204f/9efc`) and the later `05054b0e..050abffe` zero-walk (`PCTOPS=0000` while `PCTSHADOW/PCTLIVE` contain real user code). Additional post-RTE low-PC code-host discriminator windows remain default-off: `B2_JIT_LOW12B_CODEHOST=1`, `B2_JIT_LOW83_CODEHOST=1`, and `B2_JIT_LOW7F_CODEHOST=1`. `LOW83+LOW7F` was useful for the earlier oracle comparison that moved native past the stale `00008334/op=2010` stream to the oracle `00007f72` target, but it is not a desktop fix and is no longer the latest completed-run frontier.
- noisy 68040 MMU exception-frame logging is gated behind `B2_JIT_TRACE_MMU_FRAME=1`; the prefetch-guard mismatch trace is separately gated behind `B2_JIT_TRACE_PREFETCH_GUARD=1`; `B2_JIT_PCTRACE_LIVE=1` is opt-in because live addrbank reads can have side effects or fault; `B2_JIT_TRACE_LOW33_PCP=1`/`B2_JIT_TRACE_LOWPC_PCP=1` are bridge-only, non-mutating diagnostics that log host `pc_p` words beside data-view words for low33/all low-PC post-RTE faults; `B2_JIT_TRACE_LOWPC_RESUME=1` logs bounded pre/post-`Exception(2)` low-PC resume state (USP/ISP/MSP, saved JIT restart state, flags, and stack words) for the current low-PC/RTE churn; `B2_JIT_TRACE_LOWPC_COMPILE=1` is a default-off trace/compile audit that records the bridge low-PC fault sequence, logs immediate trace-time pre-fetch/post-fetch host words as `JIT_LOWPC_TRACE_OP` (respecting `B2_TRACE_PC_START/END` when set), and compares `pc_hist` host words with the post-fetch host pointer/words as `JIT_LOWPC_COMPILE` when the next block is built, without changing block identity or opcode fetch semantics; low-PC data faults now refresh the 68040 frame effective-address word from the actual bus fault address while leaving code-fetch faults distinct
