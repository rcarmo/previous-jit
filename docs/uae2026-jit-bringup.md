...
- `PREVIOUS_UAE2026_JIT_RAM=1` enables the experimental RAM/MMU dispatch mode. RAM mode no longer auto-drops to the interpreter at the RTE/page-fault seam; the conservative desktop-boot oracle is explicit via `B2_JIT_RTE_FAULT_HANDOFF=1`. The native 100%-JIT path still does not reach the desktop; the current completed discriminator reaches user space and loops at the `050069cc` dispatch seam faulting on `504f2472`.
...
Latest translated-execution debug checkpoint (2026-05-20):
- opcode harness passes: `total=62`, `jit_ok=62`, `pass=62`, `fail=0`, `infra_fail=0`, `score=100` (`/workspace/tmp/previous-opcode-harness-20260520-180203`)
- default/ROM JIT smoke passes and stayed stable for 60 seconds: `desktop_reached=1`, `stable_reached=1`, `jit_ram_requested=0`, `jit_ram_dispatch_seen=0`, `jit_last_pc=0100bb08` in `/workspace/tmp/previous-jit-doc-update-default-20260520-185848`
...
- automatic RAM-mode RTE handoff was removed so `PREVIOUS_UAE2026_JIT_RAM=1` now preserves the native 100%-JIT path unless `B2_JIT_RTE_FAULT_HANDOFF=1` is explicitly set. A no-handoff 300s smoke confirmed no `RTE fault handoff to interpreter`/`JIT_FALLBACK` log entries (`/workspace/tmp/previous-jit-no-auto-handoff-ram-20260522-091833`, `desktop_reached=0`, `jit_ram_dispatch_seen=1`, `handoff-count=0`). The current full native frontier is the repeated user-space `050069cc` fault at `addr=504f2472` with `A0=504f2452`, not an interpreter drop.
...
- current native-resume candidate keeps target-fetch canonicalization narrow to the confirmed user `05027706` `JSR (A0)` seam and adds bridge PC advancement for confirmed non-restartable byte-store faults at `0500b6ae` (`MOVE.B D2,(A0)`) and `0500bc98` (`MOVE.B (A2)+,(A0)`). The corresponding normal side-effect shapes are now covered by RAM-code fast vectors; bounded no-handoff watchdogs still may stop in the normal `0409f5xx` kernel idle/SCSI polling region before reaching those later user seams, so this is not a native-RAM desktop fix yet.
- post-RTE low-virtual opcode fetch now uses `Uae2026JitMmuXlateCodeHost()` and reads from the code-translated host page only for the isolated post-kernel low-PC window (`vbr=040ae61c`, `00003300..00003400`, non-identity `pc_p`). This avoids the stale `00003334` data-view stream (`0200/0c80`) that disagreed with `PCTSHADOW=204f/9efc` without perturbing early ROM/low-overlay SCSI boot.
- noisy 68040 MMU exception-frame logging is gated behind `B2_JIT_TRACE_MMU_FRAME=1`; the prefetch-guard mismatch trace is separately gated behind `B2_JIT_TRACE_PREFETCH_GUARD=1`; `B2_JIT_PCTRACE_LIVE=1` is opt-in because live addrbank reads can have side effects or fault
...
4. Add minimal targeted regressions for the confirmed RAM/MMU patterns: BSR target-fetch fault after return push, `RTS`/`RTR` target fetch after return pop, RTE return-code fetch after SR/A7 switch, and the handoff-disabled `050abffe` native-resume/code-shadow loop.
5. Audit RTE/page-fault native resume without conflating `fault_pc`/`instruction_pc` and `mmu_fault_addr`; RAM/MMU helper calls now force a full live-state flush before helper-delivered `Exception(2)`, but all translated code still needs explicit transaction metadata for irreversible side effects before faultable accesses.
