...
- `PREVIOUS_UAE2026_JIT_RAM=1` enables the experimental RAM/MMU dispatch mode. RAM mode now records true RAM dispatch and boots to a stable desktop by default through a conservative RTE/page-fault handoff to the exact interpreter. The remaining native-resume bug is reproducible with `B2_JIT_RTE_FAULT_HANDOFF_DISABLE=1`, which disables that handoff and currently stalls during fsck/root transition with repeated low-user/RAM-boundary faults around `000000de` and `050abffe`.
...
Latest translated-execution debug checkpoint (2026-05-19):
- opcode harness passes: `total=62`, `jit_ok=62`, `pass=62`, `fail=0`, `infra_fail=0`, `score=100` (`/workspace/tmp/previous-opcode-harness-20260519-163752`)
- default/ROM JIT smoke passes and stayed stable for 60 seconds: `desktop_reached=1`, `stable_reached=1`, `jit_ram_requested=0`, `jit_ram_dispatch_seen=0`, `jit_last_pc=0100bb08` in `/workspace/tmp/previous-jit-rte-handoff-default-retry-20260519-164317`
...
- default RAM-requested mode now reaches true RAM dispatch and boots to a stable desktop through a conservative RTE/page-fault handoff to the exact interpreter (`/workspace/tmp/previous-jit-rte-disable-knob-ram-pass-20260519-173759`: `desktop_reached=1`, `stable_reached=1`, `jit_ram_dispatch_seen=1`, `jit_dispatch_lines=6598`)
- `B2_JIT_RTE_FAULT_HANDOFF_DISABLE=1` disables that default handoff and is the current native-resume discriminator. With it enabled, RAM mode gets past the older `00003352`, `00003964/A2=2`, `init exited with 212`, and `bad exception stack format` frontiers, but stalls during fsck/root transition with repeated low-user/RAM-boundary faults around `000000de` and `050abffe` (`/workspace/tmp/previous-jit-rte-disable-baseline-ram-20260519-165201`).
...
4. Add minimal targeted regressions for the confirmed RAM/MMU patterns: BSR target-fetch fault after return push, `RTS`/`RTR` target fetch after return pop, RTE return-code fetch after SR/A7 switch, and the handoff-disabled `000000de`/`050abffe` native-resume loop.
