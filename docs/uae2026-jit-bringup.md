...
- guard-on RAM discriminator still sees repeated `00003352` exceptions, but progresses past them to `root on sd@` while continuing to execute native low-virtual code; the latest run reported `jit_ram_dispatch_seen=1`, `jit_dispatch_lines=169836`, `jit_last_pc=0405840c`, no fetched/compiled opcode mismatches, and no desktop (`/workspace/tmp/previous-jit-prefetch-guard-audit-ram-20260514-151804`)
