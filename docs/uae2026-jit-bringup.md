...
- `PREVIOUS_UAE2026_JIT_RAM=1` enables the experimental RAM/MMU dispatch mode. RAM mode now records true RAM dispatch and boots to a stable desktop by default through a conservative RTE/page-fault handoff to the exact interpreter. The remaining native-resume bug is reproducible with `B2_JIT_RTE_FAULT_HANDOFF_DISABLE=1`, which disables that handoff. Narrow JSR call-push transaction producers now move the handoff-disabled path past the repeated `000000de` loop; the latest completed discriminator stalls later around `050abffe` RAM-boundary/code-fetch faults.
...
- default RAM-requested mode now reaches true RAM dispatch and boots to a stable desktop through a conservative RTE/page-fault handoff to the exact interpreter (`/workspace/tmp/previous-jit-doc-update-ram-handoff-20260520-190438`: `desktop_reached=1`, `stable_reached=1`, `jit_ram_dispatch_seen=1`, `jit_dispatch_lines=6598`, `jit_last_pc=04089c08`)
- `B2_JIT_RTE_FAULT_HANDOFF_DISABLE=1` disables that default handoff and is the current native-resume discriminator. With it enabled, RAM mode gets past the older `00003352`, `00003964/A2=2`, `init exited with 212`, and `bad exception stack format` frontiers. Narrow JSR call-push transaction producers for `0000003e`, `00003c26`, and `0000c52c` remove the repeated `000000de` loop in the latest completed run (`/workspace/tmp/previous-jit-3c26-jsr-txn-disable-ram-20260520-172348`); the remaining completed-run frontier is a repeated RAM-boundary/code-fetch fault around `050abffe` / `050ac000`.
...
- generated/native `jit_op_rte()` routes through the exact interpreter RTE implementation; default RAM mode hands bridge-caught RTE/page-fault seams to the interpreter unless `B2_JIT_RTE_FAULT_HANDOFF_DISABLE=1` is set
