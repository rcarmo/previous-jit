...
- post-RTE low-virtual opcode fetch now uses `Uae2026JitMmuXlateCodeHost()` and reads from the code-translated host page for low-virtual/non-identity RAM/MMU PCs. This removed the stale `00003334` data-view stream (`0200/0c80`) that disagreed with `PCTSHADOW=204f/9efc` and caused the immediate `00003344` / `fault_pc=0000333e` native-resume fault.
