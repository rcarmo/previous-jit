...
- full opcode harness passes: `total=75`, `jit_ok=75`, `pass=75`, `fail=0`, `infra_fail=0`, `score=100` (`/workspace/tmp/previous-opcode-harness-20260526-022641`)
- RAM-code MMU fast smoke passes from `B2_TEST_ADDR=0x04008000`: `total=32`, `pass=32`, `fail=0`, `score=100` (`/workspace/tmp/previous-mmu-fast-smoke-20260526-022427`)
- default/ROM JIT smoke reaches the desktop in the latest no-DC check (`/workspace/tmp/previous-jit-movem-cm-default-retry-20260526-023642`, `desktop_reached=1`, `harness_rc=0`)
...
- explicit handoff RAM-requested mode reaches the desktop when `B2_JIT_RTE_FAULT_HANDOFF=1` is set (`/workspace/tmp/previous-jit-movem-cm-ram-handoff-20260526-024129`: `desktop_reached=1`, `jit_ram_requested=1`, `harness_rc=0`)
- automatic RAM-mode RTE handoff remains removed, so `PREVIOUS_UAE2026_JIT_RAM=1` preserves the native 100%-JIT path unless `B2_JIT_RTE_FAULT_HANDOFF=1` is explicitly set. Native no-handoff is not fixed. The old `000042f8` / `addr=0000020c` loop was traced to `0000aef4` MOVEM continuation-frame effective-address corruption and is now gone; clean no-handoff low42 trace `/workspace/tmp/previous-jit-movem-cm-clean-nohandoff-low42-20260526-025942` enters `000042e2` with `A3=0001aa00` and progresses to later `040017b0/040017b2` and RTE/low-PC churn before timing out.
...
- recent native-resume fixes kept target-fetch canonicalization narrow to the confirmed user `05027706` `JSR (A0)` seam, added bridge PC advancement for confirmed non-restartable byte-store faults at `0500b6ae`/`0500bc98`, and now preserve the 68040 `MMU_SSW_CM` MOVEM continuation effective address instead of replacing it with `mmu_fault_addr`. The corresponding normal side-effect shapes are covered by RAM-code fast vectors; the old low42 `A3=0` symptom is cleared, and the best current frontier is the later no-handoff `040017b0/040017b2` plus RTE/low-PC churn.
...
4. Add minimal targeted regressions for the confirmed RAM/MMU patterns: BSR target-fetch fault after return push, `RTS`/`RTR` target fetch after return pop, RTE return-code fetch after SR/A7 switch, MOVEM continuation-mode effective-address preservation, and the current later no-handoff `040017b0/040017b2` frontier.
