...
- `PREVIOUS_UAE2026_JIT_RAM=1` enables the experimental RAM/MMU dispatch mode. RAM mode now records true RAM dispatch, gets past the earlier `00003352` probe loop and the later repeated `00003964`/`A2=00000002` loop, and reaches `root on sd@`, but remains blocked before desktop; the current discriminator is a post-root `init exited with 212` path after a fixed BSR target-fetch restart seam.
...
- current audit/fix tranche keeps the bounded handoff re-enable experiment reverted: `B2_JIT_RTE_FAULT_HANDOFF=1` remains the oracle, but there is no permanent `B2_JIT_RTE_FAULT_HANDOFF_REENABLE` path because it resumes too late to isolate the low-PC seam cleanly. New low-PC interpreter tracing is env-gated with `B2_INTERP_LOWPC_TRACE` (`START`/`END`/`LIMIT`) and optional `B2_INTERP_LOWPC_TRACE_PHYS`; the trace avoids `MakeSR()` and therefore does not mutate CPU state while logging.
- bad `pc_p` recovery in `execute_normal()`/`exec_nostats()` now uses code-space `Uae2026JitMmuXlateCodeHost()` when RAM/MMU dispatch is enabled, instead of direct `get_real_address()`, so low user virtual PCs do not silently rebind to stale overlay shadow bytes. `B2_JIT_EXACT_EXEC_PCS` is available as an env-gated exact-execution range discriminator.
- latest code-fetch validation (2026-05-15): low-PC oracle comparison showed `GET_OPCODE/get_iword()` in the JIT execute loops could still return stale low-virtual probe words (`0200 0001 ...`) even when the MMU code translation, live memory, and executable shadow all held the interpreter code stream (`204f 9efc ...`). RAM/MMU low-virtual opcode fetch in `execute_normal()`/`exec_nostats()` now uses `Uae2026JitMmuFetchOpcode()`, and `Uae2026JitMmuXlateCodeHost()` syncs translated physical RAM code from the live addrbank path into the executable shadow before returning a host PC. Follow-up audit made the code-shadow sync helper local, added odd-address safety, and publishes a full restart snapshot before faultable RAM/MMU code translation/fetch. Validation: opcode harness `/workspace/tmp/previous-opcode-harness-codefetch-audit-20260515-140005` (`pass=62 fail=0 score=100`); default/ROM stable smoke `/workspace/tmp/previous-jit-codefetch-audit-default-20260515-140532` (`desktop_reached=1 stable_reached=1`, `jit_last_pc=0100bb08`); plain RAM mode without guard still gets past the repeated `00003352`/`addr=00000008` low-PC fault and reaches `root on sd@`, then times out in the same post-root scheduler/lookup region (`/workspace/tmp/previous-jit-codefetch-audit-ram-20260515-141105`, sampled PCs include `0406135a/040614a8`, no desktop yet).
...
- latest BSR target-fetch rollback fixes the `00003372 -> 00012b04` restart seam that double-pushed a return address and caused the repeated `00003964`/`A2=00000002` failure; next work should isolate the newer post-root `init exited with 212` divergence, currently seen in a JIT-only user-mode `000129c6/c53c/ba00/06162` path
