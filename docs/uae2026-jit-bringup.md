...
- `PREVIOUS_UAE2026_JIT_RAM=1` enables the experimental RAM/MMU dispatch mode. RAM mode no longer auto-drops to the interpreter at the RTE/page-fault seam; the conservative desktop-boot oracle is explicit via `B2_JIT_RTE_FAULT_HANDOFF=1`. The native 100%-JIT path still does not reach the desktop. Commit `9441c84` aligns fallback BSR call-push transaction metadata by passing producer-side decoded targets into the bridge, but the latest long native no-handoff run still times out after RTE/low-PC churn (`00012052`, `00005030`, `0000a7a8`, `00004492`, `04001ae6`).
...
- recent native-resume fixes kept target-fetch canonicalization narrow to the confirmed user `05027706` `JSR (A0)` seam, added bridge PC advancement for confirmed non-restartable byte-store faults at `0500b6ae`/`0500bc98`, and preserve the 68040 `MMU_SSW_CM` MOVEM continuation effective address instead of replacing it with `mmu_fault_addr`. The corresponding normal side-effect shapes are covered by RAM-code fast vectors; the old low42 `A3=0` symptom is cleared. The best current frontier remains native no-handoff RTE/low-PC churn; a sampled `0409f592/0409f5cc` high-kernel polling loop did not move under `B2_JIT_EXACT_EXEC_PCS=0x0409f570-0x0409f5d0` and is treated as a symptom.
