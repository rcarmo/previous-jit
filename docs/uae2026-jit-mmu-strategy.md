...
- The native resume path remains unfixed but has moved: after code-shadow, low-user call transaction fixes, and default-off low-PC code-host discriminators, the best grounded comparison now matches the explicit-handoff oracle through nine low-PC catches and diverges immediately after the matched `00008b24` data fault with an extra native-only `0000ee58` catch (`addr=0001402a`). The earlier `050abffe` / `050ac000` RAM-boundary/code-fetch loop and the `050069cc` / `504f2472` dispatch fault are no longer the current completed-run frontier. `B2_JIT_TRACE_LOWPC_RESUME=1` is the default-off bridge diagnostic for this seam; it logs pre/post-`Exception(2)` low-PC state without changing resume behavior.
