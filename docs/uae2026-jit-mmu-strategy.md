...
- Exacting all low virtual code or kernel text, global optlev0, flush-each-op, and the sampled high-kernel `0409f592..0409f5d0` polling window did not move earlier/current frontiers. A targeted `04067400..04067600` exact-exec discriminator also did not change the outcome in `/workspace/tmp/previous-jit-exact-pctrace-040674-nohandoff-20260527-124743` (`desktop_reached=0`, `jit_last_pc=04067500`). The next fix should come from the principled MMU transaction/restart model rather than another broad exact-exec discriminator.
