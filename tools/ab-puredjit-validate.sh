...
WORK="$(mktemp -d /workspace/tmp/abtest-XXXXXX)"
LOG="$WORK/boot.log"
SAVE_LOG="${SAVE_LOG:-/workspace/tmp/abtest-boot.log}"
...
echo "--- cache_hit / dispatch / exec_normal extract ---"
grep 'JIT_DIAG t=' "$LOG" | tail -1 | grep -oE 'dispatch=[0-9]+|exec_normal=[0-9]+|cache_hit=[0-9]+|compile=[0-9]+|recomp=[0-9]+|flush_hard=[0-9]+|pc=0x[0-9a-f]+' | tr '\n' ' '; echo
echo "--- last 3 distinct pc= ---"
grep -oE 'pc=0x[0-9a-f]+' "$LOG" | tail -20 | sort -u | tail -5
