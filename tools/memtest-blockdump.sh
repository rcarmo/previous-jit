...
LOG="$WORK/boot.log"
SAVE_LOG="${SAVE_LOG:-/workspace/tmp/memtest-blockdump-boot.log}"
...
HOME="$WORK/home" SDL_AUDIODRIVER=dummy \
  ...
  B2_JIT_DROP_LINK_PROD=1 B2_JIT_DROP_RET_PROD=1 B2_JIT_DROP_BSR_PROD=1 B2_JIT_DROP_JSR_PROD=1 \
  B2_JIT_BARRIER_STATS=1 \
  B2_JIT_BLOCKDUMP=1 \
  B2_JIT_DROP_BCC_PROD_LO=0x01000000 \
  B2_JIT_DROP_BCC_PROD_HI=0x05000000 \
  "$BIN" >"$LOG" 2>&1 & EMU_PID=$!
...
echo "--- JIT_BARRIER_STATS (jsr_jmp count should fall toward 0 if JSR(An) compiles) ---"
grep 'JIT_BARRIER_STATS' "$LOG" | tail -2
echo "--- NextBus frontier markers (01001ca6 / F2FFFFF0) ---"
grep -ciE '01001ca6|F2FFFFF0|NextBus|nextbus' "$LOG"
echo "--- compile/avg_block ---"
grep -oE 'compile=[0-9]+\([^)]*\)' "$LOG" | tail -1
